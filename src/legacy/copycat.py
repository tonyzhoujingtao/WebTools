...
def main():
    ...
    links = link_collector.collect('http://javarevisited.blogspot.sg/',
                                  'href="(http://javarevisited.blogspot.com/[0-9][0-9][0-9][0-9]/[0-9][0-9]/.+?.html)')
    ...
    for link in links:
        try:
            ...
            if filtered_link:
                content_generator = ContentGenerator(filtered_link)

                title = content_generator.generate_readable_title()
                content = filter_content(content_generator.generate_readable_article())

                blogger_service.create_post(title, content, False)
